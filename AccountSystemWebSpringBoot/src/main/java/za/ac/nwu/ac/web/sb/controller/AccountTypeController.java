...
public class AccountTypeController
{
    ...
    public AccountTypeController(FetchAccountTypeFlow fetchAccountTypeFlow,
                                 @Qualifier("createAccountTypeFlowName") CreateAccountTypeFlow createAccountTypeFlow, UpdateAccountTypeFlow updateAccountTypeFlow) {
    ...
    //TODO: Changes the Mnemonic for given AccountTypeName
    @PutMapping("/putNewMnemoic/{typeID}" )
    @ApiOperation(value =  "Changes the Mnemonic for given AccountTypeName", notes = "Changes the Mnemonic for given AccountTypeName")
    @ApiResponses(value = {
            @ApiResponse(code = 200, message = "FOUND"),
            @ApiResponse(code = 400, message = "Bad Request", response = GeneralResponse.class),
            @ApiResponse(code = 404, message = "Resource not found", response = GeneralResponse.class),
    ...
    public ResponseEntity<GeneralResponse<AccountTypeDto>> DecreaseAmount(
            @ApiParam(value = "Changes the Mnemonic for given AccountTypeName",
                    example = "",
                    name = "typeID",
                    required = true)
            @PathVariable("typeID") final Long typeID){

        AccountTypeDto accountType = updateAccountTypeFlow.setNewMnemonic(typeID);
        GeneralResponse<AccountTypeDto> response = new GeneralResponse<>(true, accountType);
        return new ResponseEntity<>(response, HttpStatus.OK);
    }
